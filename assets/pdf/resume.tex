...
for defending and attacking large language and vision-language models under Prof. Jun Sakuma. Conducted theoretical research on AI security and privacy.}
...
\textbf{pdft: Trainable Sparse Bases for Image Compression}
  \hfill {\em Dec. 2025-Present} \\
\textcolor{textgray}{Authored \textbf{pdft}, a JAX library learning trainable sparse bases that replace the DFT/DCT, optimized on the unitary manifold so the transform stays exactly invertible (QFT, entangled-QFT, TEBD, MERA, DCT-IV bases). Trained bases store Quick Draw images in roughly 20\% fewer bytes than JPEG's $8\times8$ block cosine transform at equal quality (arXiv:2608.00053). Supersedes the Julia original ParametricDFT.jl, ported to feature parity against committed goldens. \href{https://github.com/zazabap/pdft}{lib} \href{https://github.com/zazabap/pdft-benchmarks}{bench}} \\
